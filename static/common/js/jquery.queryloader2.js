...
OverlayLoader.prototype.createOverlay = function () {
	...
	$("<div id='qLbg'></div>").css({
		height: "104px",
		marginTop: "-" + (this.parent.options.barHeight / 2) + "px",
		background: "url(/common/images/loading.png)",
		width: "248px",
		position: "absolute",
		top: "50%",
		left: "50%",
...
OverlayLoader.prototype.updatePercentage = function (percentage) {
	this.loadbar.stop().animate({
		height: 371-(371*percentage/100)
	}, 200);

	//update textual percentage
	if (this.parent.options.percentage == true) {
		this.percentageContainer.text(percentage + "%");
	}
};
...
PreloadImage.prototype.completeLoading = function () {
    ...
    var percentage = Math.floor( (this.parent.imageDone / this.parent.imageCounter) * 10) *10;
...
QueryLoader2.prototype.onLoadComplete = function() {
	if (this.options.completeAnimation == "grow") {
		...
		this.overlayLoader.container.delay(1000).fadeOut(1000, function () {
			this.parent.destroyContainers();
			this.parent.options.onComplete();
		});
	}
